function [ H1, f, c ] = trifbank( M, K, R, fs, hz2mel, mel2hz )
...
%
%   Inputs
%           M is the number of filters, i.e., number of rows of H  M=20
%
%           K is the length of frequency response of each filter K = nfft/2+1;  
...
%
%           W2H is a warped scale to Hertz scale function handle
%
%   Outputs
...
%儿研所心音数据%
    ...
    f = linspace( f_min, f_max, K ); % frequency range (Hz), size 1xK K=nfft/2+1;129
    fw = hz2mel( f );
    ...
    for m = 1:M %1:20
        ...
        k = f>=c(m)&f<=c(m+1); % up-slope
        H1(m,k) = (f(k)-c(m))/(c(m+1)-c(m));
        k = f>=c(m+1)&f<=c(m+2); % down-slope
        H1(m,k) = (c(m+2)-f(k))/(c(m+2)-c(m+1));
