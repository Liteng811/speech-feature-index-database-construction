...
emin=ones(nfr10,1);
NESEG = 200; % 2s segment 
if nfr10 < NESEG; NESEG=nfr10; end
for i=1:floor(nfr10/NESEG)
	[eY,eI]=sort(e((i-1)*NESEG+1:i*NESEG)); % rank frame energy for each 2s segment 
	emin((i-1)*NESEG+1:i*NESEG)=eY(floor(NESEG*0.1)); % ranked at 10% of lowest energy 
    if i~=1
        emin((i-1)*NESEG+1:i*NESEG)=0.9*emin((i-1)*NESEG)+0.1*emin((i-1)*NESEG+1); % forgetting factor of 0.9
    end
end
if i*NESEG~=nfr10
...
for i=2:nfr10
    ...
    if postsnr(i)<0
        postsnr(i)=0; % set negative a posteriori SNR to 0 
    end 
...
Dsmth = zeros(nfr10,1);
for i=1:nfr10
    Dsmth(i)=sum(Dexp(i:i+Dexpl+Dexpr)); % central smooth over the entire utterance  
end

for i=1:floor(nfr10/NESEG)
    Dsmth_max((i-1)*NESEG+1:i*NESEG)=max(Dsmth((i-1)*NESEG+1:i*NESEG)); % find the maximal smoothed energy in each 2s segment 
end
if i*NESEG~=nfr10
